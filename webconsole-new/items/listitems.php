...
function listitems(){
  if(checkaccess('items', 'read')){
    ...
    if (isset($_GET['item'])){
      ...
      $query = 'SELECT i.id, i.stat_type, i.name, i.weight, i.visible_distance, i.size, i.container_max_size, i.container_max_slots, i.valid_slots, i.flags, i.decay_rate, s1.name AS item_skill_id, s2.name AS item_skill_id_2, s3.name AS item_skill_id_3, i.item_bonus_1_attr, i.item_bonus_2_attr, i.item_bonus_3_attr, i.item_bonus_1_max, i.item_bonus_2_max, i.item_bonus_3_max, i.dmg_slash, i.dmg_blunt, i.dmg_pierce, i.weapon_speed, i.weapon_penetration, i.weapon_block_targeted, i.weapon_block_untargeted, i.weapon_counterblock, i.armor_hardness, i.cstr_gfx_mesh, i.cstr_gfx_icon, i.cstr_gfx_texture, i.cstr_part, i.cstr_part_mesh, i.removed_mesh, i.armorvsweapon_type, i.category_id, i.base_sale_price, i.item_type, s4.name AS requirement_1_name, i.requirement_1_value, s5.name AS requirement_2_name, i.requirement_2_value, s6.name AS requirement_3_name, i.requirement_3_value, i.item_type_id_ammo, i.spell_id_on_hit, i.spell_on_hit_prob, i.spell_id_feature, i.spell_feature_charges, i.spell_feature_timing, i.item_anim_id, i.description, i.sound, i.item_max_quality, i.equip_script, i.consume_script, i.creative_definition, i.max_charges, i.weapon_range, i.assigned_command, i.spawnable FROM item_stats i LEFT JOIN skills AS s1 ON i.item_skill_id_1=s1.skill_id LEFT JOIN skills AS s2 ON i.item_skill_id_2=s2.skill_id LEFT JOIN skills AS s3 ON i.item_skill_id_3=s3.skill_id LEFT JOIN skills AS s4 ON i.requirement_1_name=s4.skill_id LEFT JOIN skills AS s5 ON i.requirement_2_name=s5.skill_id LEFT JOIN skills AS s6 ON i.requirement_3_name=s6.skill_id WHERE id='.$id;
      $result = mysql_query2($query);
