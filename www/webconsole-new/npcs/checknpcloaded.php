...
function checknpcloaded(){
    ...
    if (checkaccess('npcs', 'read'))
    {
        ...
        while ($line = fetchSqlRow($result)) 
        {
            ...
            if ($line[7] == 0)
            {
                $loaded = '<span class="error">no</span>';
            }

            $fullname = $line[1] . " " . $line[2];
            $fullname = trim($fullname);

            echo "<TR><TD>$line[0] </TD><TD><A href='index.php?do=npc_details&sub=main&npc_id=".$line[0]."'>".$fullname."</a></TD><TD>$line[3]</TD><TD>$line[4]/$line[5]/$line[6]</TD>";
